...
class CapGainBody extends StatefulWidget {
  ...
  const CapGainBody({Key? key, required this.res, required this.originCSV})
      : super(key: key);
...
class CapGainBodyState extends State<CapGainBody> {
  ...
  final List<List<String>> _reduceTaxHouseList = [
    [
      ...
      '20001231',
      '취득일',
      '1986.1.1~2000.12.31 신축된 5호 이상의 주택을 5년이상 임대한 국민주택 또는 세액감면대상 주택, 86년이전 신축 공동주택(입주사실 없는)'
    ],
    [
      '19990101',
      '20991231',
      '취득일',
      '1999.8.20~2001.12.31 신축된 1호이상의 주택을 포함한 2호이상의 주택을 5년이상 임대한 신축임대주택, 세액감면대상 주택'
    ],
    [
      '19951101',
      '19981231',
      '계약일',
      '1995.11.1~1997.12.31 1998.3.1~1998.12.31 서울외 지방의 미분양 국민주택 취득후 5년이상 임대한후에 양도'
    ],
    ...
    [
      ...
      '20100211',
      '계약일',
      '2009.2.12(비거주자 3.16)~2010.2.11 취득 서울시 밖에 미분양주택, 자기건설신축주택'
    ],
    ...
    [
      ...
      '20111231',
      '계약일',
      '2011.3.29현재 준공후 미분양주택을 2011년까지 취득하고 5년이상 임대하고 양도'
    ],
    [
      '20120924',
      '20121231',
      '계약일',
      '2012.9.24 현재 미분양주택을 2012년안에 취득한 9억원 이하의 주택'
    ],
    [
      '20150101',
      '20151231',
      '계약일',
      '2015.1.1~12.31 까지 취득(취득가액 6억원 이하, 연면적 135m이하)하여 5년이상 임대한 주택'
    ],
    [
      '20130401',
      '20131231',
      '계약일',
      '2013.4.1~2013.12.31 신축주택(취득가 6억원 이하 or 85m이하) 미분양주택'
    ]
  ];
  ...
  final TextEditingController _rentalHouseRegistrationDateTC =
      TextEditingController();
        ...
        Row(
          children: [
            ...
            _expectedTransferDate(_transferDateTC, '20220725', (() {
              if (_stage >= 3 && _donthaveTransferDate == false) {
                return true;
              } else {
                return false;
              }
                      ...
                      items: (() {
                        if (_stage >= 4) {
                          ...
                          currentCSV = currentCSV
                              .where((element) =>
                                  element[2] == _dropDownMenuForTypeOfTransfer)
                              .toList();
  ...
  Widget _priorInheritanceHouse() {
    CustomDropDown priorDropdown = CustomDropDown(
        items: ['O', 'X'],
        selected: selectedDropDownTable['PriorInheritanceHouse'],
        widgetName: 'PriorInheritanceHouse',
        activated: _dropDownMenuForReasonOfAquistition == '상속');
    if (_dropDownMenuForReasonOfAquistition == '상속') {
      return Row(
        children: [_smallTitle('선순위 상속주택'), priorDropdown],
      );
    } else {
      return Container();
    }
    ...
    Widget rentalHouse(StateSetter _dialogSetState) {
      return ListView(
        ...
        children: [
                  ...
                  builder: (BuildContext context, BoxConstraints constraints) {
                    return DropdownButtonHideUnderline(
                      child: DropdownButton2(
                        ...
                        onChanged: (value) {
                          setState(() {
                            _dropDownMenuHavingHome_RentalHouse =
                                value as String;
                          });
                          _dialogSetState(() {
                            _dropDownMenuHavingHome_RentalHouse =
          ...
          Row(
            children: [_smallTitle('임대주택 유형'), shortRentCheckBox()],
          ),
          Row(
            children: [_smallTitle('전용면적'), myOwnArea()],
          ),
          Row(
            children: [_smallTitle('등록시 공시가격'), officialPriceWidget()],
          ),
          Row(
            children: [_smallTitle('수도권 유무'), isSeoul()],
          ),
        ],
      );
    }
  ...
  Widget isSeoul() {
    if (_tempAddr.pnu != null &&
        (_tempAddr.pnu!.substring(0, 2) == '11' ||
            _tempAddr.pnu!.substring(0, 2) == '28' ||
            _tempAddr.pnu!.substring(0, 2) == '41')) {
      return Text(
        'O',
  ...
  Future<String> fetchOfficialPrice(String pnu, String date) async {
    String baseURL =
        'https://zxwd1u54il.execute-api.ap-northeast-2.amazonaws.com/default/price_api?';

    final response = await http
        .get(Uri.parse('${baseURL}pnu=$pnu&year=${date.substring(0, 4)}'));
  ...
  Widget AcquisitionDateETC() {
    ...
    Widget whetherHavingHome() {
      if (buyDate != null &&
          buyDate!.length > 7 &&
          contractDate != null &&
          contractDate!.length > 7 &&
          _tempAddr.pnu != null) {
        return FutureBuilder(
            future: Future.wait([
              isConflict(_tempAddr.pnu!, buyDate!),
              isConflict(_tempAddr.pnu!, contractDate!)
            ]),
            builder: (BuildContext context, AsyncSnapshot snapshot) {
    ...
    Widget Cession(List<List<dynamic>> _csv) {
      if (_dropDownMenuForKindOfConcession == null) {
        ...
        List<List<dynamic>> _filtered = _csv
            .where((element) => element[6] == _dropDownMenuForKindOfConcession!)
            .toList();
        return ListView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            ...
            itemBuilder: (context, index) {
              ...
              return Row(
                children: [
                  _smallTitle(
                      _filtered[index][4].toString().replaceAll('"', '')),
    ...
    if (_stage < 6) {
      ...
      if (_dropDownMenuForTypeOfTransfer == '주택(주거용 오피스텔 포함)' &&
          _dropDownMenuForReasonOfAquistition == '매매' &&
          (_dropDownMenuForTypeOfAcquisition == '분양권(2021년 이전 취득)' ||
              _dropDownMenuForTypeOfAcquisition == '분양권(2022년 이후 취득)')) {
        return ListView(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          children: [
      ...
      } else if ((_dropDownMenuForTypeOfTransfer == '분양권(2021년 이전 취득)' ||
              _dropDownMenuForTypeOfTransfer == '분양권(2022년 이후 취득)') &&
          _dropDownMenuForReasonOfAquistition == '매매' &&
          (_dropDownMenuForTypeOfAcquisition == '분양권(2021년 이전 취득)' ||
              _dropDownMenuForTypeOfAcquisition == '분양권(2022년 이후 취득)')) {
        return ListView(
        ...
        return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (csv[index][7].length > 1 &&
                  csv[index][7] != '취득일' &&
                  csv[index][7] != '계약일' &&
                  csv[index][7] != '취득일&계약일') {
                if (acquisitionETCTCList[0].text.length > 1 &&
                    acquisitionETCTCList[1].text.length > 1) {
              ...
              if (csv[index][4] == '"계약일 당시 무주택 여부 (o,x)"') {
                ...
                if (startInheritance!.length > 7) {
                  ...
                  inheri = DateTime.parse('21001231');
                }
                DateTime transfer = DateTime.parse(_transferDateTC.text);

                int diff =
                    int.parse(inheri.difference(transfer).inDays.toString());
                if (diff <= 731) {
                  return Row(
                    children: [
                          ...
                          onTap: () {
                            ...
                            dialogSetState(() {
                              Navigator.pop(
                                  context,
                                  '${_tempAddr.roadAddr!} ${(() {
                                    if (_tempAddr.dong! == '동 없음') {
                                      return '';
                                    } else {
                                      return _tempAddr.dong!;
                                    }
                                  })()} ${_tempAddr.ho!}');
                            });
                          },
